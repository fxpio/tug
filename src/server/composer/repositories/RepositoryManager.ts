...
export class RepositoryManager {
    ...
    public async register(url: string, type?: string, res?: Response): Promise<VcsRepository> {
        const repo = await this.createVcsRepository(url, type);
        const existingRepo = await this.getRepository(repo.getUrl(), false, res);
    ...
    public async unregister(url: string, res?: Response): Promise<string> {
        const repo = await this.createVcsRepository(url);
        const existingRepo = await this.getRepository(repo.getUrl(), false, res);
    ...
    public async findRepository(packageName: string, res?: Response): Promise<VcsRepository|null> {
        const cacheRepo = RepositoryManager.getCacheRepository(packageName, res);

        if (cacheRepo) {
            return cacheRepo;
        }

        const repoData = await this.codeRepoRepo.findOne({packageName});

        if (repoData) {
            const repo = new VcsRepository(repoData, await this.configManager.get());
    ...
    public async getRepository(url: string, required: boolean = false, res?: Response): Promise<VcsRepository|null> {
        ...
        if (packageName) {
            const repo = RepositoryManager.getCacheRepository(packageName, res);
            if (repo) {
                return repo;
            }
        }

        const result = await this.codeRepoRepo.findOne({url});
        const repo = result ? new VcsRepository(result, await this.configManager.get()) : null;
    ...
    public async getAndInitRepository(url: string, force: boolean = false, res?: Response): Promise<VcsRepository|null> {
        const repo = await this.getRepository(url, false, res);
    ...
    public async initRepository(repository: VcsRepository, force: boolean = false): Promise<boolean> {
        if (force || null === repository.getPackageName() || null === repository.getRootIdentifier()) {
            const driver = repository.getDriver();
            const rootIdentifier = await driver.getRootIdentifier();

            if (await driver.hasComposerFile(rootIdentifier)) {
                const composer: LooseObject = await driver.getComposerInformation(rootIdentifier) as LooseObject;
    ...
    public async createVcsRepository(url: string, type?: string): Promise<VcsRepository> {
        const config = await this.configManager.get();
        const repo = new VcsRepository({url, type}, config);
