...
export class GitlabDriver extends VcsDriver {
    ...
    public static supports(config: Config, url: string, deep: boolean = false): boolean {
        ...
        return Object.keys(config.get('gitlab-oauth')).includes(originUrl.replace(/^www\./i, ''));
    }
    ...
    constructor(repoConfig: LooseObject, config: Config, remoteFilesystem?: RemoteFilesystem) {
        ...
        if (null === match) {
            throw new VcsDriverInvalidUrlError('Gitlab', this.url);
        }
    ...
    public getApiUrl(): string {
        return `https://${this.originUrl}/api/v4`;
    }
    ...
    public getDist(identifier: string): LooseObject {
        const url = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/archive.tar.gz?sha=${identifier}`;
    ...
    public async getComposerInformation(identifier: string): Promise<LooseObject|null> {
        if (!this.infoCache[identifier]) {
            const composer = await this.getBaseComposerInformation(identifier);
    ...
    public async getFileContent(file: string, identifier: string): Promise<string|null> {
        ...
        try {
            const resourceFile: string = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/files/${file}?ref=${encodeURIComponent(identifier)}`;
    ...
    public async getChangeDate(identifier: string): Promise<Date|null> {
        const resource = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/commits/${encodeURIComponent(identifier)}`;
    ...
    public async getTags(): Promise<LooseObject> {
        if (null === this.tags) {
            this.tags = {};
            const resourceFile = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/tags?per_page=100`;
            let resource = null;
    ...
    public async getBranches(): Promise<LooseObject> {
        if (null === this.branches) {
            this.branches = {};
            const resourceFile = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/branches?per_page=100`;
            let resource = null;
    ...
    private async fetchRootIdentifier(): Promise<void> {
        ...
        const repoDataUrl = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}`;
        ...
        if (this.repoData) {
            ...
            this.repository = this.repoData.path;
            this.isPrivate = undefined !== this.repoData.visibility && 'private' === this.repoData.visibility;
