...
export default class GitlabDriver extends VcsDriver
{
    ...
    async getFileContent(file, identifier) {
        ...
        try {
            let resource = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/files/${file}?ref=${encodeURIComponent(identifier)}`;
    ...
    async getChangeDate(identifier) {
        let resource = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/commits/${encodeURIComponent(identifier)}`;
    ...
    async getTags() {
        if (null === this.tags) {
            this.tags = {};
            let resource = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/tags?per_page=100`;
    ...
    async getBranches() {
        if (null === this.branches) {
            this.branches = {};
            let resource = `${this.getApiUrl()}/projects/${querystring.escape(`${this.owner}/${this.repository}`)}/repository/branches?per_page=100`;
